...
package com.ezdms.servlet.admin;

import com.google.gson.Gson;
import com.ezdms.bean.ConfigStoredFile;
import com.ezdms.bean.ConfigStoredOption;
import com.ezdms.bean.ConfigStoredSelect;
import com.ezdms.core.DatabaseException;
import com.ezdms.core.MimeTypeConfig;
import com.ezdms.dao.ConfigDAO;
import com.ezdms.dao.HibernateUtil;
import com.ezdms.dao.bean.Config;
import com.ezdms.servlet.admin.DatabaseQueryServlet.WorkerUpdate;
...
public class ConfigServlet extends BaseServlet {
		...
		try {
			if (ServletFileUpload.isMultipartContent(request)) {
				...
				for (Iterator<FileItem> it = items.iterator(); it.hasNext(); ) {
					...
					if (item.isFormField()) {
						...
						if (cfg.getKey() != null && cfg.getKey().startsWith("logo")) {
							...
							if (cfg.getKey().equals(com.ezdms.core.Config.PROPERTY_LOGO_LOGIN)) {
								size = "316x74>";
							} else if (cfg.getKey().equals(com.ezdms.core.Config.PROPERTY_LOGO_MOBILE)) {
								size = "161x38>";
							} else if (cfg.getKey().equals(com.ezdms.core.Config.PROPERTY_LOGO_REPORT)) {
								size = "150x35>";
							} else if (cfg.getKey().equals(com.ezdms.core.Config.PROPERTY_LOGO_TINY)) {
								size = "51x13>";
							} else if (cfg.getKey().equals(com.ezdms.core.Config.PROPERTY_LOGO_FAVICON)) {
								size = "16x16>";
							}
				...
				if (action.equals("create")) {
					...
					com.ezdms.core.Config.reload(sc, new Properties());
	...
	private void check(String userId, HttpServletRequest request, HttpServletResponse response) throws
		...
		try {
			...
			out.print("<li>");
			out.print("<b>" + com.ezdms.core.Config.PROPERTY_SYSTEM_SWFTOOLS_PDF2SWF + "</b>");
			checkExecutable(out, com.ezdms.core.Config.SYSTEM_SWFTOOLS_PDF2SWF);
			out.print("</li>");

			out.print("<li>");
			out.print("<b>" + com.ezdms.core.Config.PROPERTY_SYSTEM_IMAGEMAGICK_CONVERT + "</b>");
			checkExecutable(out, com.ezdms.core.Config.SYSTEM_IMAGEMAGICK_CONVERT);
			out.print("</li>");

			out.print("<li>");
			out.print("<b>" + com.ezdms.core.Config.PROPERTY_SYSTEM_OCR + "</b>");
			checkExecutable(out, com.ezdms.core.Config.SYSTEM_OCR);
			out.print("</li>");

			out.print("<li>");
			out.print("<b>" + com.ezdms.core.Config.PROPERTY_SYSTEM_OPENOFFICE_PATH + "</b>");
			checkOpenOffice(out, com.ezdms.core.Config.SYSTEM_OPENOFFICE_PATH);
			out.print("</li>");
	...
	private void export(String userId, HttpServletRequest request, HttpServletResponse response) throws
		...
		String fileName = "ezDMS_" + WarUtils.getAppVersion().getVersion() + "_cfg.sql";
