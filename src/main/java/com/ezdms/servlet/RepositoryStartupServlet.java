...
package com.ezdms.servlet;

import com.cybozu.labs.langdetect.DetectorFactory;
import com.cybozu.labs.langdetect.LangDetectException;
import com.ezdms.cache.CacheProvider;
import com.ezdms.cache.UserItemsManager;
import com.ezdms.cache.UserNodeKeywordsManager;
import com.ezdms.core.*;
import com.ezdms.dao.HibernateUtil;
import com.ezdms.extension.core.ExtensionManager;
import com.ezdms.kea.RDFREpository;
import com.ezdms.module.db.DbRepositoryModule;
import com.ezdms.module.db.stuff.FsDataStore;
import com.ezdms.spring.SystemAuthentication;
import com.ezdms.util.*;
import com.ezdms.util.pendtask.PendingTaskExecutor;
...
public class RepositoryStartupServlet extends HttpServlet {
	...
	private static Timer uiTimer; // Update Info (ezDMS Update Information)
	...
	public static synchronized void start() throws ServletException {
		...
		try {
			...
			String dapContent = "com.ezdms.dao.DashboardActivityDAO.purge();";
			CronTabUtils.createOrUpdate("Dashboard Activity Purge", "@daily", dapContent);

			String uisContent = "com.ezdms.cache.UserItemsManager.serialize();";
			CronTabUtils.createOrUpdate("User Items Serialize", "@hourly", uisContent);

			String ruiContent = "com.ezdms.cache.UserItemsManager.refreshDbUserItems();";
			CronTabUtils.createOrUpdate("Refresh User Items", "@weekly", ruiContent);

			String umiContent = "new com.ezdms.core.UserMailImporter().run();";
			CronTabUtils.createOrUpdate("User Mail Importer", "*/30 * * * *", umiContent);

			String tewContent = "new com.ezdms.extractor.TextExtractorWorker().run();";
			CronTabUtils.createOrUpdate("Text Extractor Worker", "*/5 * * * *", tewContent);

			// String riContent = "new com.ezdms.core.RepositoryInfo().run();";
			// CronTabUtils.createOrUpdate("Repository Info", "@daily", riContent);

			String swdContent = "new com.ezdms.core.Watchdog().run();";
			CronTabUtils.createOrUpdate("Session Watchdog", "*/5 * * * *", swdContent);

			String pptContent = "new com.ezdms.util.pendtask.PendingTaskExecutor().run();";
			CronTabUtils.createOrUpdate("Process Pending Tasks", "*/5 * * * *", pptContent);
			
			// Datastore garbage collection
			if (!Config.REPOSITORY_NATIVE && hasConfiguredDataStore) {
				String dgcContent = "new com.ezdms.module.jcr.stuff.DataStoreGarbageCollector().run();";
	...
	/**
	 * Close ezDMS and free resources
	 */
	public static synchronized void stop(GenericServlet gs) {
		if (!running) {
			throw new IllegalStateException("ezDMS not started");
		}
